...
for  i=7 to 22 '8:00 부터 22:00까지 1시간 단위로 일정을 보여주기 위해 루프
	...
	If  (i mod 2) = 0 Then 
		...
		response.write "		<a href=main.asp?menushow="&menushow&"&theme="&theme&"&d=d&F_Year="&intThisYear&"&F_Month="&intThisMonth&"&F_Day="&IntthisDay & "&m=input&sdate="&datThisday&"&shour="&i&"><span>"& i &" : 00 </span><img src=./diary/images/add.gif alt=일정추가 border=0></a> &nbsp;"&vbCR
 ...
 <table width="630" border="0">
  <tr>          
    <td width="32%" align="right">
      <a href="main.asp?menushow=<%=menushow%>&theme=<%=theme%>&d=d&F_Year=<%=year(datThisday-1)%>&F_Month=<%=month(datThisday-1)%>&F_Day=<%=day(datThisday-1)%>"> 
      <img src="./diary/images/icon01_02_pre.gif" width="58" height="19" border=0></a>
    </td>
    <td width="13%" valign="middle">
      <a href="main.asp?menushow=<%=menushow%>&theme=<%=theme%>&d=d&F_Year=<%=year(datThisday-1)%>&F_Month=<%=month(datThisday-1)%>&F_Day=<%=day(datThisday-1)%>">[<%=month(datThisday-1)%>월 <%=day(datThisday-1)%>일]</a></td>
    <td width="10%" valign="middle"></td>
    <td width="13%" valign="middle">
      <a href="main.asp?menushow=<%=menushow%>&theme=<%=theme%>&d=d&F_Year=<%=year(datThisday+1)%>&F_Month=<%=month(datThisday+1)%>&F_Day=<%=day(datThisday+1)%>">
      [<%=month(datThisday+1)%>월 <%=day(datThisday+1)%>일]</a></td>
