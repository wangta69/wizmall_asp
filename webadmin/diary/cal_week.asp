...
For i=1 to 7
	...
	response.write "	<a href=main.asp?menushow="&menushow&"&theme="&theme&"&d=d&F_Year="&year(intCday)&"&F_month="&month(intCday)&"&F_day="&day(intCday) &">"
	
	response.write "	<span>"& intPday &"("&varPweekday&")</span></a>&nbsp;<a href=main.asp?menushow="&menushow&"&theme="&theme&"&d=d&F_Year="&year(intCday)&"&F_month="&month(intCday)&"&F_day="&day(intCday)&" &m=input&sdate="&intcday&"&shour=9><img src=./diary/images/add.gif alt=일정추가 border=0></a>&nbsp;"&vbCR
...
'//////////////////   해당 시간에 해당되는 일정 쿼리해서 처리하기 시작 ////////////////////////////
		...
		If  not objRs.eof  Then
			...
			Do while not objRs.eof 
				...
				Response.Write  " <span>"&lhour&"<a href=main.asp?menushow="&menushow&"&theme="&theme&"&d=d&d="&d&"&m=view&cid="&cc_id&" onMouseOver=""view('"&cc_title&"', '"&lhour&"','"&cc_desc&"');""  onMouseOut=""noview();"" >&nbsp;"&cc_title&"</a></span><br />"&vbCR	
...
<table width="630" border="0">
  <tr>          
    <td width="32%" align="right">
      <a href="main.asp?menushow=<%=menushow%>&theme=<%=theme%>&d=w&F_Year=<%=year(datThisday-7)%>&F_Month=<%=month(datThisday-7)%>&F_Day=<%=day(datThisday-7)%>"> 
      <img src="./diary/images/icon01_02_pre.gif" width="58" height="19" border=0></a>
    </td>
    <td width="13%" valign="middle">
      <a href="main.asp?menushow=<%=menushow%>&theme=<%=theme%>&d=w&F_Year=<%=year(datThisday-7)%>&F_Month=<%=month(datThisday-7)%>&F_Day=<%=day(datThisday-7)%>">[<%=month(datThisday-7)%>월 <%=day(datThisday-7)%>일]</a></td>
    <td width="10%" valign="middle"></td>
    <td width="13%" valign="middle">
      <a href="main.asp?menushow=<%=menushow%>&theme=<%=theme%>&d=w&F_Year=<%=year(datThisday+7)%>&F_Month=<%=month(datThisday+7)%>&F_Day=<%=day(datThisday+7)%>">
      [<%=month(datThisday+7)%>월 <%=day(datThisday+7)%>일]</a></td>
