...
dim curSize, curPer, curOS, curCount
dim sqlValue
dim addSize
...
sub Manage()
	...
	'MaxCount 쿼리
	strSQL = "select top 1 vOS,count(vNum) as a from " & theTable & sqlValue & " group by vOS order by a desc"
	Set objRs	= db.ExecSQLReturnRS(strSQL, Nothing, DbConnect)
	...
	'MinCount 쿼리
	strSQL = "select top 1 vOS,count(vNum) as a from " & theTable & sqlValue & " group by vOS order by a asc"
	Set objRs	= db.ExecSQLReturnRS(strSQL, Nothing, DbConnect)
	...
	'OS별 데이터 쿼리
	strSQL = "select vOS,count(vNum) from " & theTable & sqlValue & " group by vOS order by vOS"
	Set objRs	= db.ExecSQLReturnRS(strSQL, Nothing, DbConnect)
...
<form action="./main.asp" method="post" name="Form1">
        ...
        <input type="hidden" name="theme" value="visit/visit_os">
        <table class="table_main w_default">
          <tr> 
            <td align="left"> <select name="YY" onChange="document.Form1.submit();">
              ...
              [<a href="./main.asp?menushow=<%=menushow%>&theme=visit/visit_os">전체</a>&nbsp;|&nbsp;<a href="./main.asp?menushow=<%=menushow%>&theme=visit/visit_os&YY=<%=year(date)%>">올해</a>&nbsp;|&nbsp;<a href="./main.asp?menushow=<%=menushow%>&theme=visit/visit_os&YY=<%=year(date)%>&MM=<%=month(date)%>">이번달</a>&nbsp;|&nbsp;<a href="./main.asp?menushow=<%=menushow%>&theme=visit/visit_os&YY=<%=year(date)%>&MM=<%=month(date)%>&DD=<%=day(date)%>">오늘</a>] 
        ...
        <table class="table_main w_default">
          ...
          <%end if%>
                ...
                <tr onMouseOver="rbar(this);" onMouseOut="cbar(this);">	
                  <td>&nbsp;&nbsp;<%=curOS%></td>
                  <td><%=curCount%></td>
                  <td><img src="image_visit/graph.gif" width="<%=int(curSize * addSize)%>" height="30" align="absmiddle">&nbsp;<%=curPer%>%</td>
                </tr>
                <%next%>
