...
Dim sorting, whereis, cnt, big_code, mid_code, small_code
Dim strSQL,objRs
Dim db,util,mall
...
if category <> "" then 
	big_code = CLng(right(category, 3))
	mid_code = CLng(right(category, 6))
	small_code = CLng(right(category, 9))	
end if
%>
...
<div class="space20"></div>
      ...
      <table class="table_blank">
        <form  action='./product/product7_qry.asp' method='post' name="frm">
          ...
          <tr>
            <td width="92%"><select name="product_list" size="15" style="BACKGROUND-COLOR: #FFFFFF; BORDER: #D0D0D0 1 solid; font-family:돋움; font-size:12px; color:#5E5E5E; HEIGHT: 300px; width: 100%">
                <%
Dim uid,pname,userorder,cat_name			
whereis = " where m.uid is not null "
if category <> "" then 
		whereis = whereis & " and m.category like '%" & category &"'"
end if

strSQL = "SELECT m.uid, m.pname, m.userorder, c.cat_name FROM wizmall m left join wizcategory c on m.category=c.cat_no " & whereis & " ORDER BY m.userorder ASC"
cnt = 1
Set db = new database
...
WHILE NOT objRs.EOF
	uid			= objRs("uid")
	pname		= objRs("pname")
	userorder	= objRs("userorder")
	cat_name	= objRs("cat_name")
	Response.Write( "<option value='" & uid & "'>" & cnt & " [" & cat_name & "] " & pname & "</option>"&Chr(13)&Chr(10))  
cnt = cnt+1
objRs.MOVENEXT
WEND
%>
...
if category <> "" then 
		whereis = whereis & " and category like '%" & category &"'"
end if

strSQL = "SELECT uid FROM wizmall " & whereis & " ORDER BY userorder ASC"
Set objRs	= db.ExecSQLReturnRS(strSQL, Nothing, DbConnect)
WHILE NOT objRs.EOF
	uid			= objRs("uid")
