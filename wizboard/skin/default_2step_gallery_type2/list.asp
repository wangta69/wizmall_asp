...
<%
strSQL = "select TOP " & setPageSize & " * from " & table_name & whereis & " and uid not in (SELECT TOP " & ((page - 1) * setPageSize) & " uid from " & table_name & whereis & " ORDER BY bd_idx_num desc) ORDER BY bd_idx_num desc " 
cnt = 0
...
if listfile1 <> "" then
%>				
            <a href='wizboard.asp?bmode=view&bid=<%=bid%>&gid=<%=gid%>&adminmode=<%=adminmode%>&uid=<%=uid%>&page=<%=page%>&category=<%=category%>&search_category=&search_word=&order_c=bd_idx_num&order_da=desc' class="no0"><img src="<%=getAttachedIcon(listfile1, attached_dir, "default")%>" width="150" height="113" border="0"></a>
<%
end if
%>	</td>
        </tr>
      ...
      <table width="160" border="0" cellspacing="1" cellpadding="1">
        ...
        <tr class="ft2">
          ...
          <td height="25"  width="115" bgcolor="#F3F3F3" class="dot"><a href='wizboard.asp?bmode=view&bid=<%=bid%>&gid=<%=gid%>&adminmode=<%=adminmode%>&uid=<%=uid%>&page=<%=page%>&category=<%=category%>&search_category=&search_word=&order_c=bd_idx_num&order_da=desc' class="no0"><%=subject%></a></td>
        </tr>
      </table></td>
<%	cnt = cnt + 1
